...
const all = (children, value, message) => multiple(children, value).then(results => {
...
const custom = (() => {
  var _ref = _asyncToGenerator(function* (fn, value, message, options) {
    ...
    const path = pathToString(options.path);
    throw new Error(`${path}: Validator must return 'true' if valid or an error message as a string on errors`);
  });
