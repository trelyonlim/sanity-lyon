...
&nbsp;
  ...
  const hasFilename = assetMeta &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >assetMeta.originalFilename</span>
  const hasNonFilenameMeta = assetMeta &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >Object.keys(assetMap).length &gt; 1</span>
  const {pathname} = parseUrl(url)
  const filename = hasFilename ? <span class="branch-0 cbranch-no" title="branch not covered" >assetMeta.originalFilename </span>: basename(pathname)
&nbsp;
...
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (hasNonFilenameMeta) {
<span class="cstat-no" title="statement not covered" >    await client.patch(asset._id).set(assetMeta)</span>
  }
&nbsp;
  return asset._id
}
...
async function getAssetDocumentIdForHash(client, type, sha1hash, attemptNum = 0) {
  ...
  try {
    const dataType = type === 'file' ? <span class="branch-0 cbranch-no" title="branch not covered" >'sanity.fileAsset' </span>: 'sanity.imageAsset'
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="https://istanbul.js.org/" target="_blank">istanbul</a> at Wed Apr 04 2018 10:06:39 GMT+0200 (CEST)
</div>
</div>
