...
function validateManifest(manifest, plugin) {
  ...
  ;(manifest.parts || []).forEach((part, i) => {
    ...
    if (isImplementation && !hasPart) {
      throw new Error([baseError, 'A part that has a defined `path` needs to also define either `name` or `implements`', `See ${(0, _generateHelpUrl2.default)('plugin-parts-syntax')}`].join('\n'));
    }

    if (!isDefined(part.path) && !isDefined(part.description)) {
      throw new Error([baseError, 'A part that has not defined a `path` needs to include a `description`', `See ${(0, _generateHelpUrl2.default)('plugin-parts-syntax')}`].join('\n'));
    }
...
function validatePartName(name, baseError) {
  const examples = ['Examples:', '- part:package-name/part-name', '- part:package-name/part-name-style', '', `See ${(0, _generateHelpUrl2.default)('part-name-format')}`].join('\n');
