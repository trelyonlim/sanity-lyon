...
exports.sanityBlockNodeToSlateRaw = sanityBlockNodeToSlateRaw;
exports.sanityBlockArrayToSlateDocument = sanityBlockArrayToSlateDocument;
exports.slateMarksToSanity = slateMarksToSanity;
exports.sanityMarksToSlate = sanityMarksToSlate;
...
function sanityBlockNodeToSlateRaw(input) {
  switch (input._type) {
    case 'paragraph':
...
function sanityBlockArrayToSlateDocument(sanityBlockText) {
  const raw = sanityBlockArrayToSlateRawState(sanityBlockText);
  return _slate.Raw.deserialize(raw).get('document');
}
...
function slateMarksToSanity(marks) {
  const result = [];
  marks.forEach(mark => {
...
function sanityMarksToSlate(tags) {
  return new _immutable.Set(tags.map(tag => {
    return _slate.Mark.create({ type: tag, data: {} });
  }));
}