...
function consecutiveSanitySpansToSlateText(key, spans, firstIndex) {
  ...
  const chars = (0, _flatten3.default)(spans.map(span => {
  ...
  return _slate.Text.create({
    key,
    characters: _immutable2.default.List(chars.map(({ text, marks }) => _slate.Character.create({ text, marks })))
  });
}
...
function spanAccessorsToSlateNodes(spans) {
  let joined = _immutable2.default.List();
  let i = 0;
