...
function normalizeItems(items /*: Array<DataTransferItem>*/) /*: Promise<Array<File>>*/ {
  return Promise.all(items.map(item => {
    ...
    // others
    return new Promise(resolve => item.getAsString(resolve)).then(str => str ? [new File([str], 'unknown.txt', { type: item.type })] : []);
  }));
}
...
function walk(entry /*: WebKitFileEntry*/) /*: Promise<Array<File>>*/ {
  ...
  if (entry.isDirectory) {
    ...
    return new Promise(resolve => dir.readEntries(resolve)).then(entries => entries.filter(entr => !entr.name.startsWith('.'))).then(entries => Promise.all(entries.map(walk)).then(_flatten3.default));
  }
