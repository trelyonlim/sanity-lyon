...
module.exports = workDir => {
  ...
  Object.keys(supported).forEach(pkg => {
    ...
    if (semver.satisfies(installed, deprecated[pkg])) {
      console.warn(`[WARN] ${pkg} ${deprecated[pkg]} is deprecated and will be unsupported as of the next release of Sanity. Please upgrade the ${pkg} package. Read more at ${generateHelpUrl('upgrade-react')} `);
    }
    if (!semver.satisfies(installed, supported[pkg])) {
      throw new Error(`Sanity currently only supports ${pkg}@${supported[pkg]}. Installed version: ${installed}.`);
    }
  });
};