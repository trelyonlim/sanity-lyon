...
let determineTargetDataset = (() => {
  var _ref2 = _asyncToGenerator(function* (target, context) {
  ...
  return function determineTargetDataset(_x3, _x4) {
...
exports.default = {
  ...
  action: (() => {
    var _ref = _asyncToGenerator(function* (args, context) {
      ...
      if (isUrl) {
        (0, _debug2.default)('Input is a URL, streaming from source URL');
        inputStream = yield getUrlStream(file);
      } else {
        const sourceFile = _path2.default.resolve(process.cwd(), file);
        const fileStats = yield _fsExtra2.default.stat(sourceFile).catch(function () {
          return null;
        });
        if (!fileStats) {
        ...
        inputStream = sourceIsFolder ? sourceFile : yield _fsExtra2.default.createReadStream(sourceFile);
      }
