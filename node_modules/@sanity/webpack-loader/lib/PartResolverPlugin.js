...
const isSanityPart = request => [partMatcher, configMatcher, sanityMatcher].some(match => match.test(request.request));

const PartResolverPlugin = function PartResolverPlugin(options) {
    ...
    compiler.resolvers.normal.plugin('module', function (request, callback) {
      ...
      if (configMatch) {
        ...
        const req = Object.assign({}, request, {
          request: configFor === 'sanity' ? path.join(basePath, 'sanity.json') : path.join(configPath, `${configFor}.json`)
        });
