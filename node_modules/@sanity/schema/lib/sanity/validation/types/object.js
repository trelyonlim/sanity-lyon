...
function validateFieldName(name) /*: Array<any>*/ {
  ...
  if (!VALID_FIELD_RE.test(name)) {
    return [(0, _createValidationResult.error)(`Invalid field name: "${name}". Fields can only contain characters from A-Z, numbers and underscores and should not start with a number (must pass the regular expression ${String(VALID_FIELD_RE)}).`, _createValidationResult.HELP_IDS.OBJECT_FIELD_NAME_INVALID)];
  }
...
exports.default = (typeDef, visitorContext) => {
  ...
  if (fieldsIsArray) {
    ...
    problems.push((0, _createValidationResult.error)(`The "fields" property must be an array of fields. Instead saw "${typeof typeDef.fields}"`, _createValidationResult.HELP_IDS.OBJECT_FIELDS_INVALID));
  }
  ...
  return _extends({}, typeDef, {
    fields: (fieldsIsArray ? typeDef.fields : []).map(field => {
      ...
      const _problems = _visitorContext$visit._problems,
