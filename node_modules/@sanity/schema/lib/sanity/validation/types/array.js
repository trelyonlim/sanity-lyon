...
exports.default = (typeDef, visitorContext) => {
  ...
  const problems = (0, _flatten3.default)([ofIsArray ? (0, _getDupes.getDupes)(typeDef.of, t => `${t.name};${t.type}`).map(dupes => (0, _createValidationResult.error)(`Found ${dupes.length} members with same type, but not unique names "${dupes[0].type}" in array. This makes it impossible to tell their values apart and you should consider naming them`, _createValidationResult.HELP_IDS.ARRAY_OF_NOT_UNIQUE)) : (0, _createValidationResult.error)('The array type is missing or having an invalid value for the required "of" property', _createValidationResult.HELP_IDS.ARRAY_OF_INVALID)]);
