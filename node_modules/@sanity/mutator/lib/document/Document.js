...
class Document {
  ...
  stage(mutation /*: Mutation*/, silent /*: boolean*/) /*: SubmissionResponder*/ {
    ...
    (0, _debug2.default)('Staging mutation %s (pushed to pending)', mutation.transactionId);
  ...
  considerIncoming() {
    ...
    if (this.incoming.length > 0 && _debug2.default.enabled) {
      (0, _debug2.default)('Unable to apply mutations %s', this.incoming.map(mut => mut.transactionId).join(', '));
    }
  ...
  applyIncoming(mut /*: Mutation*/) {
    ...
    if (this.anyUnresolvedMutations()) {
      ...
      if (_debug2.default.enabled) {
        ...
        (0, _debug2.default)(`Submitted txnIds: ${this.submitted.map(m => m.transactionId).join(', ')}`);
        (0, _debug2.default)(`Pending txnIds: ${this.pending.map(m => m.transactionId).join(', ')}`);
        (0, _debug2.default)(`needRebase == %s`, needRebase);
      }
      return needRebase;
    }
    (0, _debug2.default)(`Remote mutation %s arrived w/o any pending or submitted local mutations`, mut.transactionId);
    this.EDGE = this.HEAD;
    if (this.onMutation) {
      this.onMutation({
        mutation: mut,
  ...
  consumeUnresolved(txnId /*: string*/) {
    ...
    if (this.submitted.length != 0) {
      if (this.submitted[0].transactionId == txnId) {
        (0, _debug2.default)(`Remote mutation %s matches upcoming submitted mutation, consumed from 'submitted' buffer`, txnId);
      ...
      (0, _debug2.default)(`Remote mutation %s matches upcoming pending mutation, consumed from 'pending' buffer`, txnId);
      this.pending.shift();
      return false;
    }
    (0, _debug2.default)('The mutation was not the upcoming mutation, scrubbing. Pending: %d, Submitted: %d', this.pending.length, this.submitted.length);
    ...
    (0, _debug2.default)(`After scrubbing: Pending: %d, Submitted: %d`, this.pending.length, this.submitted.length);
  ...
  rebase() {
    ...
    const changed = !(0, _isEqual3.default)(this.EDGE, oldEdge);
