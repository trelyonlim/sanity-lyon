...
const productionMode = process.env.NODE_ENV === 'production' || publish.inPublish() || includes(args, '-p') || includes(args, '--production');
...
fse.readJson(manifestDir).catch(err => {
  ...
  process.exit(1);
}).then(() => sanityCheck({ dir, productionMode })).catch(err => {
