...
function verifyImplementationExists(impl, parentDirContent) {
  ...
  const found = parentDirContent.find(file => file.toLowerCase() === targetFile || file.toLowerCase() === targetJsFile);
...
function isFileOrDirectoryWithIndex(impl) {
  ...
  }).catch(() => new Error(`Part "${impl.partName}" was attempted to be implemented by "${impl.path}", ` + `which does not seem to exist. ${checkImplementationMsg(impl)}`));
}
